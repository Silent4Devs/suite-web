export default class OrgChart {
  constructor(options) {
    ...
    let that = this,
      ...
      zoomSlider = document.querySelector(opts.zoomSlider);
    this.options = opts;
    ...
    zoomSlider.addEventListener('input', this._onInput.bind(this));
  ...
  _renderEmployeeInformation() {
    ...
    title_info_text.classList.add('side');
    title_info_text.classList.add('title-nav');
    title_info_text.innerText = `${dataSourceJSON.puesto}`;

    //area
    let area_info = document.createElement('p');
    area_info.classList.add('side');
    area_info.classList.add('area-nav');
    area_info.innerText = `${dataSourceJSON.area.area}`;
    ...
    chartContainer.appendChild(div_img);
    chartContainer.appendChild(title_info);
    // chartContainer.appendChild(name_info);
    chartContainer.appendChild(title_info_text);
  ...
  _clickExportButton() {
    ...
    if (opts.typeOrgChart == 'employees') {
      if (flag) {
        ...
        let contenedorB2T = document.querySelector('.orgchart').classList.contains('b2t');
        if (contenedorL2R) {
        ...
        if (contenedorR2L) {
          document.querySelector('.orgchart').classList.remove('r2l');
          document.querySelector('.orgchart').style = "";
          document.querySelector("#zoomer").value = 70;
          document.querySelector("#output").innerHTML = 70;
          document.querySelector('.orgchart').classList.add('t2b');
        ...
        if (contenedorB2T) {
          document.querySelector('.orgchart').classList.remove('b2t');
          document.querySelector('.orgchart').style = "";
          document.querySelector("#zoomer").value = 70;
          document.querySelector("#output").innerHTML = 70;
          document.querySelector('.orgchart').classList.add('t2b');
  ...
  _setNewChartScale(chart, newScale) {
    let lastTf = window.getComputedStyle(chart).transform;
    if (chart.classList.contains('r2l')) {
      if (lastTf === 'none') {
  ...
  _onInput(event) {
    event.preventDefault();
    let slider = document.getElementById("zoomer");
    ...
    let newScale = event.target.valueAsNumber / 100;
    this._setNewChartScale(this.chart, newScale);
  }
