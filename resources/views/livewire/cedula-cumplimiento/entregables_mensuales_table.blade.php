<table>
    <thead>
        <tr>
            <th>ID</th>
            <th>Nombre</th>
            <th>Programada&nbsp;@for ($i = 0; $i < 10; $i++)&nbsp;@endfor</th>
            <th>&nbsp;&nbsp;&nbsp;Real&nbsp;@for ($i = 0; $i < 20; $i++)&nbsp;@endfor</th>
            <th>Desfase</th>
            <th>Observaciones</th>
            <th>Cumple</th>
        </tr>
            ...
            <tr>
                ...
                <td>
                    @if (!is_null($entregable->entrega_real) && !is_null($entregable->plazo_entrega_termina))
                        @if (\Carbon\Carbon::parse($entregable->plazo_entrega_termina)->diffInDays(\Carbon\Carbon::parse($entregable->entrega_real), false) < 0)
                            ...
                            días --}}
                            Sin desfase
                        @elseif(\Carbon\Carbon::parse($entregable->plazo_entrega_termina)->diffInDays(\Carbon\Carbon::parse($entregable->entrega_real),
                            false) == 0)
                            <i class="fas fa-calendar-plus" style="color:#17de8c"></i>
                            {{-- {{ \Carbon\Carbon::parse($entregable->plazo_entrega_termina)->diffInDays(\Carbon\Carbon::parse($entregable->entrega_real), false) }}
                            días --}}
