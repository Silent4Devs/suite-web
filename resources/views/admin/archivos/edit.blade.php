...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.archivos.update", [$archivo->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="carpeta_id">{{ trans('cruds.archivo.fields.carpeta') }}</label>
                <select class="form-control select2 {{ $errors->has('carpeta') ? 'is-invalid' : '' }}" name="carpeta_id" id="carpeta_id" required>
