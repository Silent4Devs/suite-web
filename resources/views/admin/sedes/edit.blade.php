            ...
            <form method="POST" action="{{ route('admin.sedes.update', [$sede->id]) }}" enctype="multipart/form-data">
                ...
                <div class="row">
                    <div class="form-group col-sm-6">
                        ...
                        <input class="form-control {{ $errors->has('sede') ? 'is-invalid' : '' }}" maxlength="255"
                            type="text" name="sede" id="sede" value="{{ old('sede', $sede->sede) }}" required>
                        @if ($errors->has('sede'))
                    ...
                    <div class="form-group col-sm-6">
                        ...
                        <div class="mb-3 input-group">
                            <div class="custom-file">
                                <input type="file" class="custom-file-input" name="foto_sedes" id="foto_sedes"
                                    accept="image/*" value="{{ $sede->foto_sedes }}" readonly>
                ...
                <div class="form-group">
                    <label class="required" for="direccion"><i
                            class="fas fa-map-marker-alt iconos-crear"></i>Dirección</label>
                    <input class="form-control {{ $errors->has('direccion') ? 'is-invalid' : '' }}" maxlength="255"
                        type="text" name="direccion" id="direccion" value="{{ old('direccion', $sede->direccion) }}"
                        required>
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control {{ $errors->has('descripcion') ? 'is-invalid' : '' }}" maxlength="500" name="descripcion"
                        id="descripcion">{{ old('descripcion', $sede->descripcion) }}</textarea>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('organizacion') ? 'is-invalid' : '' }}"
                        ...
                        @foreach ($organizacions as $id => $organizacion)
                            <option value="{{ $id }}"
                                {{ (old('organizacion_id') ? old('organizacion_id') : $sede->organizacion->id ?? '') == $id ? 'selected' : '' }}>
                                {{ $organizacion }}</option>
                        @endforeach
                    </select>
