<form method="POST" action="{{ url('admin/plan-correctivas-storeedit') }}" enctype="multipart/form-data" class="row">
    @csrf
    ...
    <div class="form-group col-12">
        ...
        <select class="form-control {{ $errors->has('estatus') ? 'is-invalid' : '' }}" name="estatus" id="estatus"
            ...
            @foreach (App\Models\PlanaccionCorrectiva::ESTATUS_SELECT as $key => $label)
                <option value="{{ $key }}" {{ old('estatus', '') === (string) $key ? 'selected' : '' }}>
                    {{ $label }}</option>
            @endforeach
        </select>
    ...
    <div class="form-group col-12 text-right">
        ...
        <a class="btn btn-primary" href="{{ route('admin.accion-correctivas.index') }}">Cancelar</a>
...
<table class="table" id="editplanact" style="font-size: 12px;">
    ...
    <tbody>
        @foreach ($PlanAccion as $item)
            <tr>
                <td>{{ $item->id }}</td>
                <td><a href="#" data-type="text" data-pk="{{ $item->id }}"
                        data-url="{{ route('admin.planaccion-correctivas.update', $item->id) }}" data-title="Actividad"
                        data-value="{{ $item->actividad }}" class="actividad" data-name="actividad">
                    </a></td>
                <td>
                    <a href="#" data-type="combodate" data-pk="{{ $item->id }}"
                        data-url="{{ route('admin.planaccion-correctivas.update', $item->id) }}"
                        data-title="Seleccione fecha" data-value="{{ $item->fechacompromiso }}" class="fechacompromiso"
                        data-name="fechacompromiso">
                    </a>
                </td>
                <td><a href="#" data-type="select" data-pk="{{ $item->id }}"
                        data-url="{{ route('admin.planaccion-correctivas.update', $item->id) }}"
                        data-title="Seleccionar estatus" data-value="{{ $item->estatus }}" class="estatus"
                        data-name="estatus">
                    </a>
                </td>
                <td>
                    <a href="#" data-type="select" data-pk="{{ $item->id }}"
                        data-url="{{ route('admin.planaccion-correctivas.update', $item->id) }}"
                        data-title="Seleccionar responsable" data-value="{{ $item->responsable_id }}"
                        class="responsable" data-name="responsable">
                    </a>
                </td>
            </tr>
        @endforeach
