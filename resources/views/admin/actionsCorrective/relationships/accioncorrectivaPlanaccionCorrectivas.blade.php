...
<div class="card">
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table
                class=" table table-bordered table-striped table-hover datatable datatable-accioncorrectivaPlanaccionCorrectivas">
                <thead>
                    <tr>
                        <th width="10">
                            ...
                            <td>
                                @can('planaccion_correctiva_show')
                                    <a class="btn btn-xs tb-btn-primary"
                                        href="{{ route('admin.planaccion-correctivas.show', $planaccionCorrectiva->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan
                                ...
                                @can('planaccion_correctiva_edit')
                                    <a class="btn btn-xs btn-info"
                                        href="{{ route('admin.planaccion-correctivas.edit', $planaccionCorrectiva->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan
                                ...
                                @can('planaccion_correctiva_delete')
                                    <form
                                        action="{{ route('admin.planaccion-correctivas.destroy', $planaccionCorrectiva->id) }}"
...
@section('scripts')
    ...
    <script>
        $(function() {
            ...
            let table = $('.datatable-accioncorrectivaPlanaccionCorrectivas:not(.ajaxTable)').DataTable({
                buttons: dtButtons
            })
