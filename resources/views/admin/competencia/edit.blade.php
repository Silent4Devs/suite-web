...
@section('content')
    ...
    <div class="card mt-4">
        @can('competencias_editar')
            ...
            <div class="card-body">
                <form method="POST" class="row" action="{{ route('admin.competencia.update', [$competencium->id]) }}"
                    enctype="multipart/form-data">
                    @method('PUT')
                    @csrf
                    <div class="form-group col-md-6">
                        <label class="required" for="nombrecolaborador_id"><i
                                class="fas fa-user iconos-crear"></i>{{ trans('cruds.competencium.fields.nombrecolaborador') }}</label>
                        <select class="form-control select2 {{ $errors->has('nombrecolaborador') ? 'is-invalid' : '' }}"
                            name="nombrecolaborador_id" id="nombrecolaborador_id" required>
                            @foreach ($nombrecolaboradors as $id => $nombrecolaborador)
                                <option value="{{ $id }}"
                                    {{ (old('nombrecolaborador_id') ? old('nombrecolaborador_id') : $competencium->nombrecolaborador->id ?? '') == $id ? 'selected' : '' }}>
                                    {{ $nombrecolaborador }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group col-md-6">
                        <label for="perfilpuesto"><i
                                class="fas fa-user-tag iconos-crear"></i>{{ trans('cruds.competencium.fields.perfilpuesto') }}</label>
                        <input class="form-control {{ $errors->has('perfilpuesto') ? 'is-invalid' : '' }}" type="text"
                            name="perfilpuesto" id="perfilpuesto"
                            value="{{ old('perfilpuesto', $competencium->perfilpuesto) }}">
                    ...
                    <div class="form-group col-12">
                        <label for="certificados"><i
                                class="far fa-file iconos-crear"></i>{{ trans('cruds.competencium.fields.certificados') }}</label>
                        <div class="needsclick dropzone {{ $errors->has('certificados') ? 'is-invalid' : '' }}"
                            id="certificados-dropzone">
                        </div>
