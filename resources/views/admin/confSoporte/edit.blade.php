    ...
    <div class="mt-4 card">
        <div class="card-body">
            <form method="POST" class="row"
                action="{{ route('admin.configurar-soporte.update', [$ConfigurarSoporteModel->id]) }}"
                ...
                <div class="form-group col-md-4">
                    ...
                    <select class="form-control" id="rol" name="rol">
                        <option value="Consultor" @if ($ConfigurarSoporteModel->rol == 'Consultor') {{ 'selected' }} @endif>Consultor
                        </option>
                        <option value="Soporte técnico" @if ($ConfigurarSoporteModel->rol == 'Soporte técnico') {{ 'selected' }} @endif>Soporte
                            técnico</option>
                    </select>
                </div>
                <div class="form-group col-md-4 col-sm-4">
                    ...
                    <select class="form-control {{ $errors->has('id_elaboro') ? 'is-invalid' : '' }}" name="id_elaboro"
                        ...
                        @foreach ($empleados as $key => $label)
                            <option data-name="{{ $label->name }}" value="{{ $label->id }}"
                                {{ old('id_elaboro', $ConfigurarSoporteModel->id_elaboro) == $label->id ? 'selected' : '' }}>
                                {{ $label->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="form-group col-md-4 col-sm-4">
                    ...
                    <select class="form-control {{ $errors->has('id_puesto') ? 'is-invalid' : '' }}" name="puesto"
                        ...
                        <option value {{ old('id_puesto', null) === null ? 'selected' : '' }}>
                            Selecciona una opción</option>
                        @foreach ($empleados as $key => $label)
                            <option data-name="{{ $label->puesto }}" value="{{ $label->id }}"
                                {{ old('id_elaboro', $ConfigurarSoporteModel->puesto) == $label->id ? 'selected' : '' }}>
                                {{ $label->puesto }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="form-group col-md-4 col-sm-4">
                    ...
                    <input class="form-control {{ $errors->has('id_telefono') ? 'is-invalid' : '' }}" type="string"
                        maxlength="10" id="id_telefono" name="telefono"
                        value="{{ old('telefono', $ConfigurarSoporteModel->telefono) }}"
                ...
                <div class="form-group col-md-4 col-sm-4">
                    ...
                    <input class="form-control {{ $errors->has('id_extension') ? 'is-invalid' : '' }}" type="string"
                        maxlength="10" id="id_extension" name="extension"
                        value="{{ old('extension', $ConfigurarSoporteModel->extension) }}"
                        style="pointer-events: none; background-color: #e9ecef; opacity: 1;">
                ...
                <div class="form-group col-md-4 col-sm-4">
                    ...
                    <input class="form-control {{ $errors->has('id_telefono_movil') ? 'is-invalid' : '' }}" type="string"
                        maxlength="10" id="id_telefono_movil" name="tel_celular"
                        value="{{ old('tel_celular', $ConfigurarSoporteModel->tel_celular) }}"
                        style="pointer-events: none; background-color: #e9ecef; opacity: 1;">
                ...
                <div class="form-group col-md-4 col-sm-4">
                    ...
                    <input class="form-control {{ $errors->has('id_email') ? 'is-invalid' : '' }}" type="string"
                        maxlength="10" id="id_email" name="correo"
                        value="{{ old('correo', $ConfigurarSoporteModel->correo) }}"
                        style="pointer-events: none; background-color: #e9ecef; opacity: 1;">
