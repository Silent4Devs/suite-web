                            ...
                            <tr>
                                ...
                                <td style=" display: flex;justify-content: center; position: relative;">
                                    @if($doc->ruta_documento)
                                        ...
                                        <label class="text-center" style="position:absolute; right: 20px; top:50px;"  data-toggle="modal" data-target="#modal_docs_{{ $doc->id }}">
    ...
    <div id="modales_versiones">
        @foreach($lista_docs as $doc)
            <div class="modal fade" id="modal_docs_{{ $doc->id }}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                ...
                <div class="modal-content">
                  ...
                  <div class="modal-body">
                    <ul class="lista_docs_archivo">
                        <li><a target="_blank" href="{{ $doc->ruta_documento }}"><strong>{{ $doc->nombre_doc }}</strong> </a> <label>Archivo Actual <i class="fa-solid fa-circle-check" style="color:#25B82B;"></i></label></li>
                        @foreach($doc->documento_versiones as $doc_version)
                            <li>
                                <a target="_blank" href="{{ $doc_version->ruta_documento }}"><strong>{{ $doc_version->documento }}</strong> | {{ $doc_version->created_at->diffForHumans() }}</a>
                                <i class="fa-solid fa-file-export restaurar_archivo" data-id="{{ $doc_version->id }}" data-expediente-id="{{ $doc->evidencia_viejo_id }}" title="Recuperar Archivo"></i>
                            </li>
                        @endforeach
                    </ul>
                  </div>
                    ...
                    Swal.fire({
                      ...
                      if (result.isConfirmed) {
                        try{
                            ...
                            const url = '{{ route("admin.empleado.edit.expediente-restaurar") }}';
