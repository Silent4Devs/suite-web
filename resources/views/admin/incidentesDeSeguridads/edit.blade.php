...
<div class="card mt-4">
    ...
    <div class="card-body">
        <form method="POST" class="row" action="{{ route("admin.incidentes-de-seguridads.update", [$incidentesDeSeguridad->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group col-12">
                <label class="required" for="folio"><i class="far fa-file-alt iconos-crear"></i>{{ trans('cruds.incidentesDeSeguridad.fields.folio') }}</label>
                <input class="form-control {{ $errors->has('folio') ? 'is-invalid' : '' }}" type="text" name="folio" id="folio" value="{{ old('folio', $incidentesDeSeguridad->folio) }}" required>
                @if($errors->has('folio'))
            ...
            <div class="form-group col-12">
                <label for="resumen"><i class="fas fa-file-alt iconos-crear"></i>{{ trans('cruds.incidentesDeSeguridad.fields.resumen') }}</label>
            ...
            <div class="form-group col-md-6">
                <label><i class="fas fa-sitemap iconos-crear"></i>{{ trans('cruds.incidentesDeSeguridad.fields.prioridad') }}</label>
                <select class="form-control {{ $errors->has('prioridad') ? 'is-invalid' : '' }}" name="prioridad" id="prioridad">
                    ...
                    @foreach(App\Models\IncidentesDeSeguridad::PRIORIDAD_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('prioridad', $incidentesDeSeguridad->prioridad) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group col-md-6">
                <label for="fechaocurrencia"><i class="far fa-calendar-alt iconos-crear"></i>{{ trans('cruds.incidentesDeSeguridad.fields.fechaocurrencia') }}</label>
                <input class="form-control date {{ $errors->has('fechaocurrencia') ? 'is-invalid' : '' }}" type="text" name="fechaocurrencia" id="fechaocurrencia" value="{{ old('fechaocurrencia', $incidentesDeSeguridad->fechaocurrencia) }}">
            ...
            <div class="form-group col-12">
                ...
                <select class="form-control select2 {{ $errors->has('activos') ? 'is-invalid' : '' }}" name="activos[]" id="activos" multiple>
                    @foreach($activos as $id => $activo)
                        <option value="{{ $id }}" {{ (in_array($id, old('activos', [])) || $incidentesDeSeguridad->activos->contains($id)) ? 'selected' : '' }}>{{ $activo }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group col-md-6">
                <label for="clasificacion"><i class="fas fa-list-ul iconos-crear"></i>{{ trans('cruds.incidentesDeSeguridad.fields.clasificacion') }}</label>
                <input class="form-control {{ $errors->has('clasificacion') ? 'is-invalid' : '' }}" type="text" name="clasificacion" id="clasificacion" value="{{ old('clasificacion', $incidentesDeSeguridad->clasificacion) }}">
            ...
            <div class="form-group col-md-6">
                ...
                <select class="form-control select2 {{ $errors->has('estado') ? 'is-invalid' : '' }}" name="estado_id" id="estado_id">
                    @foreach($estados as $id => $estado)
                        <option value="{{ $id }}" {{ (old('estado_id') ? old('estado_id') : $incidentesDeSeguridad->estado->id ?? '') == $id ? 'selected' : '' }}>{{ $estado }}</option>
                    @endforeach
                </select>
