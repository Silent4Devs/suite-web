...
<div class="card mt-4">
    <div class="col-md-10 col-sm-9 py-3 card-body verde_silent align-self-center" style="margin-top: -40px;">
        <h3 class="mb-1  text-center text-white"><strong> Registrar: </strong> Información Documentada  </h3>
    </div>
            ...
            <div class="form-group  col-md-6">
                ...
                <select class="form-control {{ $errors->has('tipodocumento') ? 'is-invalid' : '' }}" name="tipodocumento" id="tipodocumento">
                    ...
                    @foreach(App\Models\InformacionDocumetada::TIPODOCUMENTO_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('tipodocumento', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group col-12">
                ...
                <select class="form-control select2 {{ $errors->has('politicas') ? 'is-invalid' : '' }}" name="politicas[]" id="politicas" multiple>
                    @foreach($politicas as $id => $politicas)
                        <option value="{{ $id }}" {{ in_array($id, old('politicas', [])) ? 'selected' : '' }}>{{ $politicas }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group col-md-4">
                ...
                <select class="form-control select2 {{ $errors->has('aprobacion') ? 'is-invalid' : '' }}" name="aprobacion_id" id="aprobacion_id">
                    @foreach($aprobacions as $id => $aprobacion)
                        <option value="{{ $id }}" {{ old('aprobacion_id') == $id ? 'selected' : '' }}>{{ $aprobacion }}</option>
                    @endforeach
                </select>
