...
@section('content')
    ...
    @if (!is_null($organizacion))
        ...
        @if (
            $panel_rules->linkedin == false &&
                $panel_rules->logotipo == false &&
                $panel_rules->youtube == false &&
                $panel_rules->facebook == false &&
                $panel_rules->twitter == false &&
                $panel_rules->empresa == false &&
                $panel_rules->razon_social == false &&
                $panel_rules->rfc == false &&
                $panel_rules->schedule == false &&
                $panel_rules->direccion == false &&
                $panel_rules->telefono == false &&
                $panel_rules->correo == false &&
                $panel_rules->pagina_web == false)
        @elseif(
            $panel_rules->linkedin == true ||
                $panel_rules->logotipo == true ||
                $panel_rules->youtube == true ||
                $panel_rules->facebook == true ||
                $panel_rules->twitter == true ||
                $panel_rules->empresa == true ||
                $panel_rules->razon_social == true ||
                $panel_rules->rfc == true ||
                $panel_rules->schedule == true ||
                $panel_rules->direccion == true ||
                $panel_rules->telefono == true ||
                $panel_rules->correo == true ||
                $panel_rules->pagina_web == true)
            <div class="card-body card">
                <div class="row">
                    <div class="col-md-12 col-sm-12">
                        @if (
                            $panel_rules->linkedin ||
                                $panel_rules->logotipo ||
                                $panel_rules->youtube ||
                                $panel_rules->facebook ||
                                $panel_rules->twitter ||
                                $panel_rules->empresa ||
                                $panel_rules->razon_social ||
                                $panel_rules->rfc ||
                                $panel_rules->schedule ||
                                $panel_rules->direccion ||
                                $panel_rules->telefono ||
                                $panel_rules->correo ||
                                $panel_rules->pagina_web)
                            ...
                            <div class="col-{{ $panel_rules->logotipo ? '4' : '12' }}">
                                @if ($panel_rules->logotipo)
                                    <div style="margin-top:0px;transform: scale(0.7);margin-top:-40px">
                                        <img class="" src="{{ url($logotipo) }}" alt="Card image" style="width:100%;">
                                    ...
                                    @if (!is_null($organizacion->empresa))
                                        <div class="form-group col-sm-12 col-md-12">
                                            ...
                                            <input class="form-control {{ $errors->has('empresa') ? 'is-invalid' : '' }}"
                                                ...
                                                value="{{ $organizacion->empresa }}" disabled>
                                    ...
                                    @if (!is_null($organizacion->rfc))
                                        <div class="form-group col-sm-12">
                                            ...
                                            <input class="form-control {{ $errors->has('empresa') ? 'is-invalid' : '' }}"
                                                ...
                                                value="{{ $organizacion->rfc }}" disabled>
                                            @if ($errors->has('rfc'))
                                                <div class="invalid-feedback">
                                                    {{ $errors->first('rfc') }}
                                                </div>
                                    ...
                                    @if (!is_null($organizacion->direccion))
                                        <div class="form-group col-sm-12 col-md-12">
                                            ...
                                            <input class="form-control {{ $errors->has('direccion') ? 'is-invalid' : '' }}"
                                                ...
                                                value=" {{ $organizacion->direccion }}" disabled>
                                    ...
                                    @if (!is_null($organizacion->telefono))
                                        <div class="form-group col-sm-12 col-md-6">
                                            ...
                                            <input class="form-control {{ $errors->has('telefono') ? 'is-invalid' : '' }}"
                                                ...
                                                value="{{ $organizacion->telefono }}" disabled>
                                    ...
                                    @if (!is_null($organizacion->correo))
                                        <div class="form-group col-sm-12 col-md-6">
                                            ...
                                            <input class="form-control {{ $errors->has('correo') ? 'is-invalid' : '' }}"
                                                ...
                                                value="{{ $organizacion->correo }}" disabled>
                                                    ...
                                                    <div class=" row col-12 p-0 m-0">
                                                        <label class="col-md-4 col-sm-4" for="working_day"
                                                            style="text-align: center;"><i
                                                                class="fas fa-calendar-alt iconos-crear"></i>Día
                                                            Laboral</label>
                                                        <label class="col-md-4 col-sm-4" for="working_day"
                                                            style="text-align: center;"><i
                                                                class="fas fa-clock iconos-crear"></i>Horario
        ...
        @if (
            $panel_rules->representante_legal == false &&
                $panel_rules->fecha_constitucion == false &&
                $panel_rules->num_empleados == false &&
                $panel_rules->tamano == false &&
                $panel_rules->giro == false &&
                $panel_rules->servicios == false &&
                $panel_rules->mision == false &&
                $panel_rules->vision == false &&
                $panel_rules->valores == false &&
                $panel_rules->antecedentes)
        @elseif(
            $panel_rules->representante_legal == true ||
                $panel_rules->fecha_constitucion == true ||
                $panel_rules->num_empleados == true ||
                $panel_rules->tamano == true ||
                $panel_rules->giro == true ||
                $panel_rules->servicios == true ||
                $panel_rules->mision == true ||
                $panel_rules->vision == true ||
                $panel_rules->valores == true ||
                $panel_rules->antecedentes == true)
            <div class="card-body card">
                <div class="row">
                    <div class="col-md-12 col-sm-12 mb-4">
                        @if (
                            $panel_rules->representante_legal ||
                                $panel_rules->fecha_constitucion ||
                                $panel_rules->num_empleados ||
                                $panel_rules->tamano ||
                                $panel_rules->giro ||
                                $panel_rules->servicios ||
                                $panel_rules->mision ||
                                $panel_rules->vision ||
                                $panel_rules->valores ||
                                $panel_rules->antecedentes)
                    ...
                    @if ($panel_rules->representante_legal)
                        @if (!is_null($organizacion->representante_legal))
                            <div
                                class="mb-4 col-{{ $panel_rules->fecha_constitucion ? '6' : '12' }} && {{ $panel_rules->num_empleados ? '4' : '12' }}">
                    ...
                    @if ($panel_rules->num_empleados)
                        @if (!is_null($organizacion->num_empleados))
                            <div
                                class="form-group col-{{ $panel_rules->fecha_constitucion ? '6' : '6' }} && {{ $panel_rules->tamano ? '6' : '12' }}">
                                <label class="" for="num_empleados"><i class="fas fa-users iconos-crear"></i>Número
                                    de empleados</label>
                    ...
                    @if ($panel_rules->tamano)
                        @if (!is_null($organizacion->tamano))
                            <div class="form-group  col-{{ $panel_rules->num_empleados ? '6' : '12' }}">
                                ...
                                <input class="form-control {{ $errors->has('tamano') ? 'is-invalid' : '' }}"
                                    type="text" name="tamano" id="tamano" value="{{ $organizacion->tamano }}"
                                    disabled>
                        ...
                        @if (!is_null($organizacion->servicios))
                            <div class="form-group col-{{ $panel_rules->giro ? '6' : '12' }}">
                                ...
                                <input class="form-control {{ $errors->has('servicios') ? 'is-invalid' : '' }}"
                                    type="text" name="servicios" id="servicios"
                                    value="{{ $organizacion->servicios }}" disabled>
