...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.controlDocumento.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.control-documentos.update", [$controlDocumento->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="clave">{{ trans('cruds.controlDocumento.fields.clave') }}</label>
                <input class="form-control {{ $errors->has('clave') ? 'is-invalid' : '' }}" type="text" name="clave" id="clave" value="{{ old('clave', $controlDocumento->clave) }}">
                @if($errors->has('clave'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('nombre') ? 'is-invalid' : '' }}" type="text" name="nombre" id="nombre" value="{{ old('nombre', $controlDocumento->nombre) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('fecha_creacion') ? 'is-invalid' : '' }}" type="text" name="fecha_creacion" id="fecha_creacion" value="{{ old('fecha_creacion', $controlDocumento->fecha_creacion) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('version') ? 'is-invalid' : '' }}" type="text" name="version" id="version" value="{{ old('version', $controlDocumento->version) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('elaboro') ? 'is-invalid' : '' }}" name="elaboro_id" id="elaboro_id">
                    @foreach($elaboros as $id => $elaboro)
                        <option value="{{ $id }}" {{ (old('elaboro_id') ? old('elaboro_id') : $controlDocumento->elaboro->id ?? '') == $id ? 'selected' : '' }}>{{ $elaboro }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('reviso') ? 'is-invalid' : '' }}" name="reviso_id" id="reviso_id">
                    @foreach($revisos as $id => $reviso)
                        <option value="{{ $id }}" {{ (old('reviso_id') ? old('reviso_id') : $controlDocumento->reviso->id ?? '') == $id ? 'selected' : '' }}>{{ $reviso }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('estado') ? 'is-invalid' : '' }}" name="estado_id" id="estado_id">
                    @foreach($estados as $id => $estado)
                        <option value="{{ $id }}" {{ (old('estado_id') ? old('estado_id') : $controlDocumento->estado->id ?? '') == $id ? 'selected' : '' }}>{{ $estado }}</option>
                    @endforeach
                </select>
