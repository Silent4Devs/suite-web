...
@section('content')
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form method="POST" action="{{ route('admin.recursos.update', [$recurso->id]) }}"
                enctype="multipart/form-data">
                @method('PUT')
                @csrf
                <div class="form-group col-12">
                    <label for="cursoscapacitaciones"><i
                            class="fas fa-users iconos-crear"></i>{{ trans('cruds.recurso.fields.cursoscapacitaciones') }}</label>
                ...
                <div class="form-group col-12">
                    <label for="fecha_curso"><i
                            class="fas fa-calendar iconos-crear"></i>{{ trans('cruds.recurso.fields.fecha_curso') }}</label>
                    <input class="form-control  {{ $errors->has('fecha_curso') ? 'is-invalid' : '' }}" type="text"
                        name="fecha_curso" id="fecha_curso" value="{{ old('fecha_curso', $recurso->fecha_curso) }}">
                ...
                <div class="form-group col-12">
                    <label for="participantes"><i
                            class="fas fa-users iconos-crear"></i>{{ trans('cruds.recurso.fields.participantes') }}</label>
                    ...
                    <select class="form-control select2 {{ $errors->has('participantes') ? 'is-invalid' : '' }}"
                        name="participantes[]" id="participantes" multiple>
                        @foreach ($participantes as $id => $participantes)
                            <option value="{{ $id }}"
                                {{ in_array($id, old('participantes', [])) || $recurso->participantes->contains($id) ? 'selected' : '' }}>
                ...
                <div class="form-group col-12">
                    <label for="instructor"><i
                            class="fas fa-users iconos-crear"></i>{{ trans('cruds.recurso.fields.instructor') }}</label>
                ...
                <div class="form-group col-12">
                    <label for="certificado"><i
                            class="fas fa-graduation-cap iconos-crear"></i>{{ trans('cruds.recurso.fields.certificado') }}</label>
                    <div class="needsclick dropzone {{ $errors->has('certificado') ? 'is-invalid' : '' }}"
                        id="certificado-dropzone">
                    </div>
