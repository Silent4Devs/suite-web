...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.plan-base-actividades.update", [$planBaseActividade->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="actividad">{{ trans('cruds.planBaseActividade.fields.actividad') }}</label>
                <input class="form-control {{ $errors->has('actividad') ? 'is-invalid' : '' }}" type="text" name="actividad" id="actividad" value="{{ old('actividad', $planBaseActividade->actividad) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('actividad_padre') ? 'is-invalid' : '' }}" name="actividad_padre_id" id="actividad_padre_id">
                    @foreach($actividad_padres as $id => $actividad_padre)
                        <option value="{{ $id }}" {{ (old('actividad_padre_id') ? old('actividad_padre_id') : $planBaseActividade->actividad_padre->id ?? '') == $id ? 'selected' : '' }}>{{ $actividad_padre }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('ejecutar') ? 'is-invalid' : '' }}" name="ejecutar_id" id="ejecutar_id">
                    @foreach($ejecutars as $id => $ejecutar)
                        <option value="{{ $id }}" {{ (old('ejecutar_id') ? old('ejecutar_id') : $planBaseActividade->ejecutar->id ?? '') == $id ? 'selected' : '' }}>{{ $ejecutar }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('estatus') ? 'is-invalid' : '' }}" name="estatus_id" id="estatus_id">
                    @foreach($estatuses as $id => $estatus)
                        <option value="{{ $id }}" {{ (old('estatus_id') ? old('estatus_id') : $planBaseActividade->estatus->id ?? '') == $id ? 'selected' : '' }}>{{ $estatus }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('responsable') ? 'is-invalid' : '' }}" name="responsable_id" id="responsable_id">
                    @foreach($responsables as $id => $responsable)
                        <option value="{{ $id }}" {{ (old('responsable_id') ? old('responsable_id') : $planBaseActividade->responsable->id ?? '') == $id ? 'selected' : '' }}>{{ $responsable }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('colaborador') ? 'is-invalid' : '' }}" name="colaborador_id" id="colaborador_id">
                    @foreach($colaboradors as $id => $colaborador)
                        <option value="{{ $id }}" {{ (old('colaborador_id') ? old('colaborador_id') : $planBaseActividade->colaborador->id ?? '') == $id ? 'selected' : '' }}>{{ $colaborador }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('fecha_inicio') ? 'is-invalid' : '' }}" type="text" name="fecha_inicio" id="fecha_inicio" value="{{ old('fecha_inicio', $planBaseActividade->fecha_inicio) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('fecha_fin') ? 'is-invalid' : '' }}" type="text" name="fecha_fin" id="fecha_fin" value="{{ old('fecha_fin', $planBaseActividade->fecha_fin) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('compromiso') ? 'is-invalid' : '' }}" type="text" name="compromiso" id="compromiso" value="{{ old('compromiso', $planBaseActividade->compromiso) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('real') ? 'is-invalid' : '' }}" type="text" name="real" id="real" value="{{ old('real', $planBaseActividade->real) }}">
                @if($errors->has('real'))
                    <div class="invalid-feedback">
                        {{ $errors->first('real') }}
                    </div>
