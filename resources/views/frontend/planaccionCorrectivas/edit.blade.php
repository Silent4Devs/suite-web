    ...
    <div class="card">
        ...
        <div class="card-body">
            <form method="POST" action="{{ route("admin.planaccion-correctivas.update", [$planaccionCorrectiva->id]) }}"
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('accioncorrectiva') ? 'is-invalid' : '' }}"
                        ...
                        @foreach($accioncorrectivas as $id => $accioncorrectiva)
                            <option
                                value="{{ $id }}" {{ (old('accioncorrectiva_id') ? old('accioncorrectiva_id') : $planaccionCorrectiva->accioncorrectiva->id ?? '') == $id ? 'selected' : '' }}>{{ $accioncorrectiva }}</option>
                        @endforeach
                    </select>
                    ...
                    @endif
                    <span
                        class="help-block">{{ trans('cruds.planaccionCorrectiva.fields.accioncorrectiva_helper') }}</span>
                </div>
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('actividad') ? 'is-invalid' : '' }}" type="text"
                           ...
                           value="{{ old('actividad', $planaccionCorrectiva->actividad) }}" required>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('responsable') ? 'is-invalid' : '' }}"
                        ...
                        @foreach($responsables as $id => $responsable)
                            <option
                                value="{{ $id }}" {{ (old('responsable_id') ? old('responsable_id') : $planaccionCorrectiva->responsable->id ?? '') == $id ? 'selected' : '' }}>{{ $responsable }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    <label
                        for="fechacompromiso">{{ trans('cruds.planaccionCorrectiva.fields.fechacompromiso') }}</label>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control {{ $errors->has('estatus') ? 'is-invalid' : '' }}" name="estatus"
                        ...
                        @foreach(App\Models\PlanaccionCorrectiva::ESTATUS_SELECT as $key => $label)
                            <option
                                value="{{ $key }}" {{ old('estatus', $planaccionCorrectiva->estatus) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
