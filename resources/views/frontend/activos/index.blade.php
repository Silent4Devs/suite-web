        ...
        <div class="col-md-12">
            @can('activo_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.activos.create') }}">
                            ...
                            <tbody>
                                @foreach($activos as $key => $activo)
                                    <tr data-entry-id="{{ $activo->id }}">
                                        ...
                                        <td>
                                            @can('activo_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.activos.show', $activo->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('activo_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.activos.edit', $activo->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('activo_delete')
                                                <form action="{{ route('frontend.activos.destroy', $activo->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('activo_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.activos.massDestroy') }}",
...
@endcan
  ...
  let table = $('.datatable-Activo:not(.ajaxTable)').DataTable({ buttons: dtButtons })
