@extends('layouts.frontend')
@section('content')
<div class="container">
    <div class="row justify-content-center">
        <div class="col-md-12">
            @can('registromejora_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.registromejoras.create') }}">
                            {{ trans('global.add') }} {{ trans('cruds.registromejora.title_singular') }}
                        </a>
                    </div>
                </div>
            @endcan
            <div class="card">
                <div class="card-header">
                    {{ trans('cruds.registromejora.title_singular') }} {{ trans('global.list') }}
                </div>

                <div class="card-body">
                    <div class="table-responsive">
                        <table class=" table table-bordered table-striped table-hover datatable datatable-Registromejora">
                            <thead>
                                <tr>
                                    <th>
                                        {{ trans('cruds.registromejora.fields.id') }}
                                    </th>
                                    <th>
                                        {{ trans('cruds.registromejora.fields.nombre_reporta') }}
                                    </th>
                                    <th>
                                        {{ trans('cruds.registromejora.fields.nombre') }}
                                    </th>
                                    <th>
                                        {{ trans('cruds.registromejora.fields.prioridad') }}
                                    </th>
                                    <th>
                                        {{ trans('cruds.registromejora.fields.clasificacion') }}
                                    </th>
                                    <th>
                                        {{ trans('cruds.registromejora.fields.descripcion') }}
                                    </th>
                                    <th>
                                        {{ trans('cruds.registromejora.fields.responsableimplementacion') }}
                                    </th>
                                    <th>
                                        {{ trans('cruds.registromejora.fields.participantes') }}
                                    </th>
                                    <th>
                                        {{ trans('cruds.registromejora.fields.recursos') }}
                                    </th>
                                    <th>
                                        {{ trans('cruds.registromejora.fields.beneficios') }}
                                    </th>
                                    <th>
                                        {{ trans('cruds.registromejora.fields.valida') }}
                                    </th>
                                    <th>
                                        &nbsp;
                                    </th>
                                </tr>
                                <tr>
                                    <td>
                                    </td>
                                    <td>
                                        <input class="search" type="text" placeholder="{{ trans('global.search') }}">
                                    </td>
                                    <td>
                                        <select class="search">
                                            <option value>{{ trans('global.all') }}</option>
                                            @foreach($users as $key => $item)
                                                <option value="{{ $item->name }}">{{ $item->name }}</option>
                                            @endforeach
                                        </select>
                                    </td>
                                    <td>
                                        <input class="search" type="text" placeholder="{{ trans('global.search') }}">
                                    </td>
                                    <td>
                                        <select class="search" strict="true">
                                            <option value>{{ trans('global.all') }}</option>
                                            @foreach(App\Models\Registromejora::PRIORIDAD_SELECT as $key => $item)
                                                <option value="{{ $item }}">{{ $item }}</option>
                                            @endforeach
                                        </select>
                                    </td>
                                    <td>
                                        <input class="search" type="text" placeholder="{{ trans('global.search') }}">
                                    </td>
                                    <td>
                                        <input class="search" type="text" placeholder="{{ trans('global.search') }}">
                                    </td>
                                    <td>
                                        <select class="search">
                                            <option value>{{ trans('global.all') }}</option>
                                            @foreach($users as $key => $item)
                                                <option value="{{ $item->name }}">{{ $item->name }}</option>
                                            @endforeach
                                        </select>
                                    </td>
                                    <td>
                                        <input class="search" type="text" placeholder="{{ trans('global.search') }}">
                                    </td>
                                    <td>
                                        <input class="search" type="text" placeholder="{{ trans('global.search') }}">
                                    </td>
                                    <td>
                                        <input class="search" type="text" placeholder="{{ trans('global.search') }}">
                                    </td>
                                    <td>
                                        <select class="search">
                                            <option value>{{ trans('global.all') }}</option>
                                            @foreach($users as $key => $item)
                                                <option value="{{ $item->name }}">{{ $item->name }}</option>
                                            @endforeach
                                        </select>
                                    </td>
                                    <td>
                                    </td>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach($registromejoras as $key => $registromejora)
                                    <tr data-entry-id="{{ $registromejora->id }}">
                                        <td>
                                            {{ $registromejora->id ?? '' }}
                                        </td>
                                        <td>
                                            {{ $registromejora->nombre_reporta->name ?? '' }}
                                        </td>
                                        <td>
                                            {{ $registromejora->nombre ?? '' }}
                                        </td>
                                        <td>
                                            {{ App\Models\Registromejora::PRIORIDAD_SELECT[$registromejora->prioridad] ?? '' }}
                                        </td>
                                        <td>
                                            {{ $registromejora->clasificacion ?? '' }}
                                        </td>
                                        <td>
                                            {{ $registromejora->descripcion ?? '' }}
                                        </td>
                                        <td>
                                            {{ $registromejora->responsableimplementacion->name ?? '' }}
                                        </td>
                                        <td>
                                            {{ $registromejora->participantes ?? '' }}
                                        </td>
                                        <td>
                                            {{ $registromejora->recursos ?? '' }}
                                        </td>
                                        <td>
                                            {{ $registromejora->beneficios ?? '' }}
                                        </td>
                                        <td>
                                            {{ $registromejora->valida->name ?? '' }}
                                        </td>
                                        <td>
                                            @can('registromejora_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.registromejoras.show', $registromejora->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('registromejora_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.registromejoras.edit', $registromejora->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('registromejora_delete')
                                                <form action="{{ route('frontend.registromejoras.destroy', $registromejora->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
                                                    <input type="hidden" name="_method" value="DELETE">
                                                    <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                                    <input type="submit" class="btn btn-xs btn-danger" value="{{ trans('global.delete') }}">
                                                </form>
                                            @endcan

                                        </td>

                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
                    </div>
                </div>
            </div>

        </div>
    </div>
</div>
@endsection
@section('scripts')
@parent
<script>
    $(function () {
  let dtButtons = $.extend(true, [], $.fn.dataTable.defaults.buttons)
@can('registromejora_delete')
  let deleteButtonTrans = '{{ trans('global.datatables.delete') }}'
  let deleteButton = {
    text: deleteButtonTrans,
    url: "{{ route('frontend.registromejoras.massDestroy') }}",
    className: 'btn-danger',
    action: function (e, dt, node, config) {
      var ids = $.map(dt.rows({ selected: true }).nodes(), function (entry) {
          return $(entry).data('entry-id')
      });

      if (ids.length === 0) {
        alert('{{ trans('global.datatables.zero_selected') }}')

        return
      }

      if (confirm('{{ trans('global.areYouSure') }}')) {
        $.ajax({
          headers: {'x-csrf-token': _token},
          method: 'POST',
          url: config.url,
          data: { ids: ids, _method: 'DELETE' }})
          .done(function () { location.reload() })
      }
    }
  }
  dtButtons.push(deleteButton)
@endcan

  $.extend(true, $.fn.dataTable.defaults, {
    orderCellsTop: true,
    order: [[ 1, 'desc' ]],
    pageLength: 100,
  });
  let table = $('.datatable-Registromejora:not(.ajaxTable)').DataTable({ buttons: dtButtons })
  $('a[data-toggle="tab"]').on('shown.bs.tab click', function(e){
      $($.fn.dataTable.tables(true)).DataTable()
          .columns.adjust();
  });
  $('.datatable thead').on('input', '.search', function () {
      let strict = $(this).attr('strict') || false
      let value = strict && this.value ? "^" + this.value + "$" : this.value
      table
        .column($(this).parent().index())
        .search(value, strict)
        .draw()
  });
})

</script>
@endsection