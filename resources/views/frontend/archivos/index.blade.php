        ...
        <div class="col-md-12">
            @can('archivo_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.archivos.create') }}">
                            ...
                            <tbody>
                                @foreach($archivos as $key => $archivo)
                                    <tr data-entry-id="{{ $archivo->id }}">
                                        ...
                                        <td>
                                            @can('archivo_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.archivos.show', $archivo->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('archivo_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.archivos.edit', $archivo->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('archivo_delete')
                                                <form action="{{ route('frontend.archivos.destroy', $archivo->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('archivo_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.archivos.massDestroy') }}",
...
@endcan
  ...
  let table = $('.datatable-Archivo:not(.ajaxTable)').DataTable({ buttons: dtButtons })
