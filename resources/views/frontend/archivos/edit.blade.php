    ...
    <div class="row justify-content-center">
        <div class="col-md-12">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.archivos.update", [$archivo->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group">
                            <label class="required" for="carpeta_id">{{ trans('cruds.archivo.fields.carpeta') }}</label>
                            <select class="form-control select2" name="carpeta_id" id="carpeta_id" required>
                                @foreach($carpetas as $id => $carpeta)
                                    <option value="{{ $id }}" {{ (old('carpeta_id') ? old('carpeta_id') : $archivo->carpeta->id ?? '') == $id ? 'selected' : '' }}>{{ $carpeta }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <div class="needsclick dropzone" id="nombre-dropzone">
                            </div>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="estado_id" id="estado_id">
                                @foreach($estados as $id => $estado)
                                    <option value="{{ $id }}" {{ (old('estado_id') ? old('estado_id') : $archivo->estado->id ?? '') == $id ? 'selected' : '' }}>{{ $estado }}</option>
                                @endforeach
                            </select>
