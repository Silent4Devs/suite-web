...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.gapTre.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.gap-tres.update", [$gapTre->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="pregunta">{{ trans('cruds.gapTre.fields.pregunta') }}</label>
                <input class="form-control {{ $errors->has('pregunta') ? 'is-invalid' : '' }}" type="text" name="pregunta" id="pregunta" value="{{ old('pregunta', $gapTre->pregunta) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('valoracion') ? 'is-invalid' : '' }}" name="valoracion" id="valoracion">
                    ...
                    @foreach(App\Models\GapTre::VALORACION_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('valoracion', $gapTre->valoracion) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('evidencia') ? 'is-invalid' : '' }}" type="text" name="evidencia" id="evidencia" value="{{ old('evidencia', $gapTre->evidencia) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('recomendacion') ? 'is-invalid' : '' }}" type="text" name="recomendacion" id="recomendacion" value="{{ old('recomendacion', $gapTre->recomendacion) }}">
