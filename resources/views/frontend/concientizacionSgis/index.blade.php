        ...
        <div class="col-md-12">
            @can('concientizacion_sgi_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.concientizacion-sgis.create') }}">
                            {{ trans('global.add') }} {{ trans('cruds.concientizacionSgi.title_singular') }}
                        </a>
                    </div>
                </div>
            @endcan
            <div class="card">
                <div class="card-header">
                    {{ trans('cruds.concientizacionSgi.title_singular') }} {{ trans('global.list') }}
                </div>

                <div class="card-body">
                    <div class="table-responsive">
                        <table class=" table table-bordered table-striped table-hover datatable datatable-ConcientizacionSgi">
                            <thead>
                                <tr>
                                    <th>
                            ...
                            <tbody>
                                @foreach($concientizacionSgis as $key => $concientizacionSgi)
                                    <tr data-entry-id="{{ $concientizacionSgi->id }}">
                                        <td>
                                        ...
                                        <td>
                                            @can('concientizacion_sgi_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.concientizacion-sgis.show', $concientizacionSgi->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('concientizacion_sgi_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.concientizacion-sgis.edit', $concientizacionSgi->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('concientizacion_sgi_delete')
                                                <form action="{{ route('frontend.concientizacion-sgis.destroy', $concientizacionSgi->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('concientizacion_sgi_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.concientizacion-sgis.massDestroy') }}",
...
@endcan
  ...
  let table = $('.datatable-ConcientizacionSgi:not(.ajaxTable)').DataTable({ buttons: dtButtons })
