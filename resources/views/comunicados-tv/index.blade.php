...
<body>
    ...
    <ul id="slider">
        <li class=""><img src="{{ asset('img/Carrusel_inicio.png') }}"
                onerror="this.src='{{ asset('img/Carrusel_inicio.png') }}'; this.onerror=null;"></li>
        @forelse($comunicacionSgis_carrusel as $carrusel)
            @php
                ...
                $tipo_archivo = $carrusel->imagenes_comunicacion->first()
                    ? $carrusel->imagenes_comunicacion->first()->tipo
                    : '';
            @endphp
            <li class="" data-tipo="{{ $tipo_archivo }}">
                @if ($tipo_archivo == 'video')
                    <video muted controls
                        onerror="this.style.display='none'; document.getElementById('errorImage{{ $carrusel->id }}').style.display='block'">
                        <source src="{{ asset($imagen) }}" type="video">
                    </video>
                    <img id="errorImage{{ $carrusel->id }}" src="{{ asset('img/Carrusel_inicio.png') }}" alt="Error"
                        style="display: none;">
                @else
        ...
        @empty
            <li class=""><img src="{{ asset('img/tabantaj_fondo_blanco.webp') }}"
                    onerror="this.src='{{ asset('img/Carrusel_inicio.png') }}'; this.onerror=null;"></li>
        @endforelse
    </ul>
