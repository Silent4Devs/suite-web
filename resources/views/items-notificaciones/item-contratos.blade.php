<div class="d-flex align-items-center justify-content-between">
    <div style="{{ $place == 'notificaciones-page' ? 'flex-basis: calc(80% - 5px)' : 'flex-basis:100%' }}">
        <a class="dropdown-item text-secondary"
            href="{{ route('contract_manager.contratos-katbol.show', $last_unread_notification->data['id']) }}">
            @switch(" ".$last_unread_notification->data['type']) {{-- Se concatena un espacio porque el autoformateado lo agrega en el case --}}
                @case(' create')
                    <div class="d-flex align-items-center justify-content-start">
                        ...
                        @endif
                        &nbsp;&nbsp;
                        <p class="p-0 m-0">El {{ $last_unread_notification->data['slug'] }} {{ $last_unread_notification->data['id']}}  a solicitado su firma</p>
                    </div>
                @break
