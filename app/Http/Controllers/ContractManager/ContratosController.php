...
class ContratosController extends AppBaseController
{
    ...
    public function store(Request $request)
    {
        $validatedData = $request->validate([
            ...
            'monto_pago' => 'required|max:99999999999.99',
            // 'minimo' => ['required', "regex:/(^[$](?!0+\\\.00)(?=.{1,14}(\.|$))(?!0(?!\.))\d{1,3}(,\d{3})*(\.\d{1,2})?)/"],
            'minimo' => 'required|max:99999999999.99',
            // 'maximo' => ['required', "regex:/(^[$](?!0+\\\.00)(?=.{1,14}(\.|$))(?!0(?!\.))\d{1,3}(,\d{3})*(\.\d{1,2})?)/"],
            'maximo' => 'required|max:99999999999.99',
