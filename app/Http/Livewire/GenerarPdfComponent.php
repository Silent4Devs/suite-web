...
use App\Http\Livewire\GenerarPdfComponent as LivewireGenerarPdfComponent;
...
class GenerarPdfComponent extends Component
{
    ...
    public function generarEstudioContexto($filename)
    {
        ...
        $documentProtection->setPassword(GenerarPdfComponent::PASSWORD_WORD);
        ...
        $texto_negrita = array('bold' => true, 'color' => '000000');
        $texto_alinear_centro = array('alignment' => Jc::CENTER);
        $texto_justificado = array('alignment' => Jc::BOTH);
        ...
        $table->addRow();
        $table->addCell(4000)->addImage(public_path($logotipo), array(
            'width'         => 70,
            'alignment'    => JC::CENTER,
            'valign' => 'center'
        ));

        $table->addCell(8000, array(
            'valign' => 'center'
        ...
        $contexto_externo_it
            ->addText('Entorno externo en el que la organización busca alcanzar sus objetivos.' . GenerarPdfComponent::NUEVA_LINEA . '
        ...
        $table_generales->addCell(null, array('gridSpan' => 2, 'valign' => 'center'))->addText('DATOS GENERALES', null, $texto_alinear_centro);
        $table_generales->addRow();
        ...
        $table_marco_legal->addCell(14000, array('gridSpan' => 9, 'valign' => 'center'))->addText('MATRIZ DE REQUISITOS LEGALES Y REGULATORIOS', null, $texto_alinear_centro);
        $table_marco_legal->addRow();
