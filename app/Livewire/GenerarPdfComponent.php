...
class GenerarPdfComponent extends Component
{
    ...
    public function generarDocumento($valor = 'contexto')
    {
        ...
        if (! $organizacion) {
            $this->dispatch('showErrorAlert', 'Aún no has registrado tu organización');
        ...
        if (! $foda) {
            $this->dispatch('showErrorAlert', 'Aún no has realizado en análisis FODA');
        ...
        switch ($valor) {
            case 'Contexto de la organización':
                ...
                if (! $control_documento) {
                    $this->dispatch('showErrorAlert', msg:'No existe el control de documento:'.$control_documento->nombre);
                    ...
                    if (is_null($control_documento->clave) || is_null($control_documento->fecha_creacion) || is_null($control_documento->elaboro_id) || is_null($control_documento->reviso_id)) {
                        $this->dispatch('showErrorAlert', msg:'Tienes que llenar los campos faltantes del control de documento: '.$control_documento->nombre);
                    ...
                    $this->dispatch('showSuccessAlert', 'Documento PDF y Word generados con éxito');
                }
                break;
            case 'Test':
                $control_documento = ControlDocumento::where('nombre', '=', 'Test')->first();
                $this->dispatch('showSuccessAlert', 'En construcción');
                break;
            default:
                $this->dispatch('showErrorAlert', msg:'Algo salió, no existe el control de documento con el nombre: '.$this->nombre_documento);
                break;
        }
        ...
        switch ($valor) {
            case 'contexto':
                ...
                if (! $control_documento) {
                    ...
                    $this->dispatch('showSuccessAlert', 'Archivo Word Generado con éxito');
                }
                break;
            default:
