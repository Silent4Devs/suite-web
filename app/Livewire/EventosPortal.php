...
class EventosPortal extends Component
{
    ...
    public function render()
    {
        ...
        $this->nuevos = Cache::remember('Portal:Portal_nuevos', 3600 * 6, function () use ($hoy) {
        ...
        $this->cumpleaños = Cache::remember('Portal:Portal_cumpleaños', 3600 * 6, function () use ($hoy, $getAlta) {
            return $getAlta->whereMonth('cumpleaños', '=', $hoy->format('m'))->get();
        });

        $this->cumpleaños_contador_circulo = $this->cumpleaños->count();

        $this->aniversarios = Cache::remember('Portal:Portal_aniversarios', 3600 * 6, function () use ($hoy, $getAlta) {
