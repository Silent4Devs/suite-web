...
class SolicitudPermisoListener implements ShouldQueue
{
     ...
     * @return void
     */
    public function handle(SolicitudPermisoEvent $event)
    {
        try {
            ...
            if ($user) {
                ...
                if ($supervisor_empleado) {
                    ...
                    Notification::send($supervisor_usuario, new SolicitudPermisoNotification($event->permiso, $event->tipo_consulta, $event->tabla, $event->slug));
                }
            }
        } catch (\Throwable $th) {
            // throw $th;
